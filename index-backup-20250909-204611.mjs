...
function extractProperties(page) {
  ...
  return {
    ...
    area: props.Area?.select?.name?.toLowerCase() || null, // No default - must be explicitly set
...
async function createTaskInSupabase(pageData, notionPageId, hash) {
  ...
  return await retryWithBackoff(async () => {
    const { data, error } = await supabase.rpc('create_task_from_capture_with_area', {
      ...
      p_status: pageData.status,
      p_focus_slot: pageData.focusSlot,
      p_focus_date: pageData.focusDate,
  ...
  try {
    ...
    for (const page of pages) {
      try {
        ...
        // Skip if no area assigned (waiting for user to complete)
        if (!props.area || props.area === '') {
          log.debug('Skipping page without area assignment', { 
            id: page.id, 
            title: props.title 
          });
          skipped++;
